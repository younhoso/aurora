  ...
  return (
    <>
      ...
      >
        ...
        <IconSVG fill={isActive ? '#f92626' : '#fff'} strokeWidth={2} size={40} name="fill-heart" />
      </LikeBtn>
...
const ThumbItemBox = ({ className, children, isListType }) => {
  return isListType ? (
    <div className={`thumb-item-box ${className ? `${className}__thumb` : ''}`}>{children}</div>
  ) : (
    <>{children}</>
  );
};
...
const ProductGrid = ({ className, style, displayType, products }) => {
  const isListType = displayType === THUMB_LIST_TYPE.LIST;
    ...
    <ThumbList style={style} displayType={displayType} className={className}>
      {products.map(
        ({
          ...
          frontDisplayYn && (
            <ThumbItemBox key={productNo} className={className} isListType={isListType}>
              <ThumbItem
